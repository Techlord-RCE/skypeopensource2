...
static void dump_41_list (const skype_list *list)
{
	...
	for (i = 0, l = 0; i < list->things; i++)
	{
		...
		{
			debuglog_notime("%02X-%02X: ",list->thing[i].type, list->thing[i].id);
...
int main_unpack (u8 *indata, u32 inlen) {
	...
	while ((packed_bytes>0) && (blob_pos[0]!=0x41)){ 			
		...
		i++;
	    if (i > inlen){
			debuglog_notime("AES DECODING ERROR!!!\n");
			return -1;
		};
	};
...
static void dump_41_list_getdata1 (const skype_list *list, u8 *cred, u8 *rnd64bit, u32 *sess_id)
{
	...
	for (i = 0, l = 0; i < list->things; i++) {
		...
		debuglog_notime("Type:0x%08X\n",list->thing[i].type);
		debuglog_notime("Id:0x%08X\n",list->thing[i].id);
		debuglog_notime("m:0x%08X\n",list->thing[i].m);
		debuglog_notime("n:0x%08X\n",list->thing[i].n);
		debuglog_notime("\n");
	...
	for (i = 0, l = 0; i < list->things; i++) {
		...
		if ((list->thing[i].type == type) && (list->thing[i].id == id)) {
			...
			*membuf_len = list->thing[i].n;
			//debuglog_notime("list->thing[i].n: %08X\n",list->thing[i].n);
			//debuglog_notime("list->thing[i].m: %08X\n",list->thing[i].m);
			//debuglog_notime("list->thing[i].type: %08X\n",list->thing[i].type);
			//debuglog_notime("list->thing[i].id: %08X\n",list->thing[i].id);
			return 1;
		};
...
static int dump_41_list_getbuf_count (const skype_list *list, int *blobs_count, int type, int id) {
	...
	for (i = 0, l = 0; i < list->things; i++) {
		...
		if ((list->thing[i].type == type) && (list->thing[i].id == id)) {
			...
			if (0) {
                debuglog_notime("blobs_count: %08X\n",*blobs_count);
            };
			//debuglog_notime("list->thing[i].n: %08X\n",*blobs_count);
			//debuglog_notime("list->thing[i].m: %08X\n",list->thing[i].m);
			//debuglog_notime("list->thing[i].type: %08X\n",list->thing[i].type);
...
static int dump_41_list_getbuf_one (const skype_list *list, u8 *membuf, int *membuf_len, int *count, int index, int type, int id) {
	...
	for (i = 0, l = 0; i < list->things; i++) {
		...
		if ((list->thing[i].type == type) && (list->thing[i].id == id)) {
			if (0){
                debuglog_notime("Count: %d\n",*count);
                debuglog_notime("Index: %d\n",index);
            };
            if (index==(*count)) {
    			memcpy(membuf,(char *)list->thing[i].m, list->thing[i].n);
    			*membuf_len = list->thing[i].n;
